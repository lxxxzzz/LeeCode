 ...
 */
typedef NS_ENUM(NSInteger, YXIMMediaType) {
 ...
 */
typedef NS_ENUM(NSInteger, YXIMPersistFlag) {
    YXIMPersistFlag_NOT_PERSIST = 0,
    YXIMPersistFlag_PERSIST = 1,
    YXIMPersistFlag_PERSIST_AND_COUNT = 3,
    YXIMPersistFlag_TRANSPARENT = 4,
};

/**
 普通消息的持久化内容
 */
@interface YXIMMessagePayload : NSObject
...
/**
 媒体消息的持久化内容
 */
@interface YXIMMediaMessagePayload : YXIMMessagePayload

/**
 媒体类型
 */
@property (nonatomic, assign)YXIMMediaType mediaType;
 ...
 */
@protocol YXIMMessageContent <NSObject>
 ...
 */
- (void)decode:(YXIMMessagePayload *)payload;
 ...
 */
- (NSString *)digest:(nullable YXIMMessage *)message;
...
/**
 消息内容，自定义消息可以继承此类
 */
@interface YXIMMessageContent : NSObject <YXIMMessageContent>
